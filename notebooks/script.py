...
for i_fold, (x_train, x_valid) in enum_folds:
    print("FOLD:", i_fold)
    print(x_train.shape)
    print(x_valid.shape)
    ...
    # tensorboard_callback_0 = tf.keras.callbacks.TensorBoard(log_dir=run_dir / f"ae{i_fold:02d}", 
    #                         histogram_freq=1, write_images=True, profile_batch=(2,10),
    #                         update_freq="batch")
    # tensorboard_callback_1 = tf.keras.callbacks.TensorBoard(log_dir=run_dir / f"nf{i_fold:02d}", 
    #                         histogram_freq=1, write_images=True, profile_batch=(2,10),
    #                         update_freq="batch")
    ...
    # SAVE MODELS
    pae.ae.save_weights(run_dir / f"ae_{i_fold:02d}.h5")
    pae.nf.save_weights(run_dir / f"nf_{i_fold:02d}.h5")

    # PLOT LOSS AND LATENT SPACE
    fig = loss_plot(pae.history)
    fig.write_image(run_dir / f"loss_plot_{i_fold:02d}.pdf")

    z_true = pae.ae.encode(dataset['x_train'])
    z_sample = pae.nf.sample(dataset['x_train'].shape[0])

    fig = latent_space_plot(z_true, z_sample)
    fig.write_image(run_dir / f"latent_space_{i_fold:02d}.pdf")
    ...
    STEPS=500
    map_summary_writer = tf.summary.create_file_writer(str(run_dir / f"map_{i_fold:02d}"))
    ...
    fig.update_layout(
        ...
        paper_bgcolor='rgba(0,0,0,1)',
            font=dict(size=18))
    fig.write_image(run_dir / f"ascore_{i_fold:02d}.pdf")
    ...
    fig.update_layout(
        ...
        paper_bgcolor='rgba(0,0,0,1)',
            font=dict(size=18))
    fig.write_image(run_dir / f"ascore2_{i_fold:02d}.pdf")
    ...
    fig.write_image(run_dir / f"pre_cut_{i_fold:02d}.pdf") 
    ...
    fig.write_image(run_dir / f"post_cut_{i_fold:02d}.pdf") 
    ...
    dump_json(js_div_lpz, run_dir / f"js_lpz_{i_fold:02d}.json")
    dump_json(js_div_mse, run_dir / f"js_mse_{i_fold:02d}.json")
    dump_json(js_div_nmse, run_dir / f"js_nsme_{i_fold:02d}.json")
    dump_json(js_div_pae, run_dir / f"js_pae_{i_fold:02d}.json")
    ...
    fig.write_image(run_dir / f"JS_{i_fold:02d}.pdf")
    ...
    fig.write_image(run_dir / f"JS2_{i_fold:02d}.pdf")
    # In[ ]:
    dump_json(js_div_lpz2, run_dir / f"js_lpz2_{i_fold:02d}.json")
    dump_json(js_div_mse2, run_dir / f"js_mse2_{i_fold:02d}.json")
    dump_json(js_div_nmse2, run_dir / f"js_nsme2_{i_fold:02d}.json")
    dump_json(js_div_pae2, run_dir / f"js_pae2_{i_fold:02d}.json")
    ...
    roc_nmse, fpr, tpr, a = make_trace(labels, score, 'cornflowerblue')
    dump_json({"fpr": fpr.tolist(), "tpr": tpr.tolist(), "auc": a}, run_dir / f"roc_nmse_{i_fold:02d}.json")
    score = -pae.log_prob_encoding(dataset['x_test'])
    roc_lpz, fpr, tpr, a = make_trace(labels, score, 'chocolate')
    dump_json({"fpr": fpr.tolist(), "tpr": tpr.tolist(), "auc": a}, run_dir / f"roc_lpz_{i_fold:02d}.json")
    fig = go.Figure()
    ...
    fig.write_image(run_dir / f"roc_{i_fold:02d}.pdf")

    roc_lpz2, fpr, tpr, a = make_trace(labels, lp2, 'coral')
    dump_json({"fpr": fpr.tolist(), "tpr": tpr.tolist(), "auc": a}, run_dir / f"roc_lpz2_{i_fold:02d}.json")
    roc_pae2, fpr, tpr, a = make_trace(labels, ascore2, 'indigo')
    dump_json({"fpr": fpr.tolist(), "tpr": tpr.tolist(), "auc": a}, run_dir / f"roc_pae2_{i_fold:02d}.json")
    roc_mse2, fpr, tpr, a = make_trace(labels, mse2, 'royalblue')
    dump_json({"fpr": fpr.tolist(), "tpr": tpr.tolist(), "auc": a}, run_dir / f"roc_mse2_{i_fold:02d}.json")
    roc_nmse2, fpr, tpr, a = make_trace(labels, mses2, 'deepskyblue')
    dump_json({"fpr": fpr.tolist(), "tpr": tpr.tolist(), "auc": a}, run_dir / f"roc_nmse2_{i_fold:02d}.json")
    ...
    fig.update_yaxes(scaleanchor="x", scaleratio=1)
    fig.update_xaxes(constrain='domain')
    fig.write_image(run_dir / f"roc2_{i_fold:02d}.pdf")
