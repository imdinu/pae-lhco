...
if __name__ == '__main__':
    ...
    files = {
        ...
        'bbox':'../data/BBOX3_bkgHLF_merged.h5',
    }
    ...
    plt.legend()
    plt.savefig("./plots/mjj_density_bbox3.png")
    ...
    plt.savefig("./plots/mjj_weights_bbox3.png")
    plt.show(block=False)  
    ...
    loss_plot(pae.history, save_path="./plots/train_bbox3.png")
    z_true = pae.ae.encode(train['x_train'])
    z_sample = pae.nf.sample(train['x_train'].shape[0])

    latent_space_plot(z_true, z_sample, save_path='plots/latent_space_bbox3.png')

    mse = pae.reco_error(train['x_train'])
    pae.compute_implicit_sigma(train['x_valid'])
    ascore = -pae.anomaly_score(train['x_train'])

    mjj_cut_plot(mse, train['mjj_train'], prc=80, score_name='MSE', save_path='./plots/mse_cut_bkg_bbox3.png')
    ...
    sculpting_plot(ano_scores, train['mjj_train'], max_prc=98, save_path='./plots/mass_sculpting_bkg_bbox3.png')

    ascore_test = -pae.anomaly_score(test['x_test'])
    bkg, data = mjj_cut_plot(ascore_test, test['mjj_test'], prc=98, score_name='NLL', bins=80, save_path='./plots/cut_bbox3_samescaler.png')

    import pyBumpHunter as BH
    
    weights = np.repeat(1/(bkg.shape[0]/data.shape[0]),bkg.shape[0])
    hunter = BH.BumpHunter(rang=(2900,4500),
                        width_min=3,
                        width_max=5,
                        width_step=1,
                        scan_step=1,
    ...
    hunter.PlotBump(data,bkg,filename='./plots/bump_bbox3.png')
